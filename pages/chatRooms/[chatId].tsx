    ...
    return (
        <div className='flex justify-center items-center pt-20 h-screen '>
            <div className='h-[35rem] sm:h-[38rem] w-[45rem] rounded-lg shadow-lg px-5 py-3 flex flex-col'>
                ...
                <div className='relative flex-1 flex flex-col mt-1 overflow-scroll'>
                    <div className='flex-1 flex flex-col w-full gap-1 overflow-y-scroll'>
                        {
                            chats?.map((chat: MessageType) => (
                                <div className={`${userData?.user?.email === chat.email ? "self-end" : "self-start"}`} key={chat.message}>
                                    <ChatChip email={chat.email} message={chat.message} />
                                </div>
                            ))
                        }
                    </div>
                    <div className='flex items-center justify-between w-full'>
                        <input type="text" ref={messageRef} placeholder='Enter something...' className='border-none outline-none border-b-2 w-full' />
